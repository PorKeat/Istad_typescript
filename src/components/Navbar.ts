import { fetchProductById, fetchProductsBySearch } from "../api";
...
function setupNavbarFunctionality(navbar: HTMLElement) {
  // Initialize dark mode
  initializeDarkMode();

  const mobileMenuBtn = navbar.querySelector(
    "#mobileMenuBtn"
  ) as HTMLButtonElement;
  const mobileMenu = navbar.querySelector("#mobileMenu") as HTMLElement;
  ...
  const darkModeToggle = navbar.querySelector(
    "#darkModeToggle"
  ) as HTMLButtonElement;
  ...
  const searchInput = navbar.querySelector("#searchInput") as HTMLInputElement;
  const resultList = navbar.querySelector("#searchResults") as HTMLElement;
  ...
  if (searchInput && resultList) {
    searchInput.addEventListener("input", (e) => {
      ...
      debounceTimer = setTimeout(async () => {
        ...
        if (!isNaN(productId)) {
          const matched = await fetchProductById(productId);
...
function updateActiveNav(currentRoute: string) {
...
window.addEventListener("popstate", () => {
  ...
  window.dispatchEvent(
    new CustomEvent("navigate", { detail: { route: window.location.pathname } })
  );
});

export { navigateTo, handleSearch };
