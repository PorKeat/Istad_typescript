...
export async function renderProductDetails(): Promise<HTMLElement> {
  ...
  const createErrorDiv = (title: string, message: string, retry = false) => {
      ...
      <div class="space-y-6">
        ...
        <div class="space-y-2">
          <div class="flex items-baseline gap-4">
            <span class="text-4xl font-bold text-black dark:text-white">$${product.price.toFixed(
              2
            )}</span>
            ${
              savings > 0
                ? `<span class="text-xl text-gray-400 dark:text-gray-500 line-through">$${originalPrice.toFixed(
                    2
                  )}</span>`
                : ""
            }
          ...
          ${
            savings > 0
              ? `<p class="text-sm text-gray-600 dark:text-gray-400">Save $${savings.toFixed(
                  2
